...
$modVersion = ([xml](Get-Content -LiteralPath (Join-Path $PSScriptRoot 'StackAll.csproj') -Raw)).Project.PropertyGroup.Version
...
dotnet build (Join-Path $PSScriptRoot 'StackAll.csproj') -c Release --nologo "-p:GameDir=$GameDir"
if ($LASTEXITCODE) { throw 'Stack build failed.' }
dotnet run --project (Join-Path $PSScriptRoot 'tests/ContractChecks.csproj') -c Release "-p:GameDir=$GameDir" '-p:BaseIntermediateOutputPath=obj/Contracts/' -- $GameDir (Join-Path $PSScriptRoot 'bin/Release/netstandard2.1/OldMarket.StackAll.dll')
if ($LASTEXITCODE) { throw 'Game patch contract checks failed.' }
$taskOutput = Join-Path $taskRoot 'outputs'
New-Item -ItemType Directory -Force $taskOutput | Out-Null
$taskZip = Join-Path $taskOutput "OldMarket.StackAll-$modVersion.zip"
