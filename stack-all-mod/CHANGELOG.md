...
- 新增 G 专门丢出当前格的空可复用容器：短按一个，长按 0.6 秒后每 0.12 秒一个。保留满盒、半满盒及其他格；本地化提示在空盒耗尽时立即变灰，菜单、失焦、其他库存操作和切换格子会取消连扔。版本保持 0.2.2，仅 Stack All 迁移到游戏 2.1.6 / SDK r9，补齐经核对的输入与 Harmony 字段。不改变存档/RPC 格式，新操作与提示仍待实机和联机验证。
