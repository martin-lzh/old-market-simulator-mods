...
These files are Navigation Mod resources for Old Market Simulator 2.1.6. The release ZIP installs them under `BepInEx/plugins/OldMarket.Navigation/maps/` alongside the plugin. [map-pack.json](../map-pack.json) lists the eleven runtime files and their SHA256 hashes; this maintenance note is not a runtime map.
...
这些文件属于 Navigation Mod，适用于游戏 2.1.6。发行 ZIP 将其放在 `BepInEx/plugins/OldMarket.Navigation/maps/`，与插件一同安装。[map-pack.json](../map-pack.json) 明确列出十一个运行时文件及 SHA256；本维护说明不作为运行时地图打包。
