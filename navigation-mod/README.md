...
Verified on the 2.1.6 baseline: SDK r6 and real-reference Release builds match in symbolic IL, assembly references and embedded resources; 894 pure checks, 489 read-only installed-assembly contracts and 22 CI tooling tests pass. No Unity game code was executed by these checks.
...
固定游戏 **2.1.6**、Unity Mono **2022.3.62f3** 与编译 SDK **2.1.6/r6**，既有 Mod 保留 r1。CI 使用官方固定哈希的 BepInEx 5.4.23.5 引用；反射使用的存档槽、区域与扩建成员纳入 SDK 和真实程序集契约检查。无游戏环境可运行上文 `ci.py validate`、CI 工具测试和 `ci.py build`；匹配的本机安装可用 `ci.py verify-game --game-dir <path>` 核对真实引用符号 IL/资源与契约，不启动游戏。
...
2.1.6 基线验证：SDK r6 与真实引用 Release 构建的符号 IL、程序集引用及内嵌资源一致；894 项纯逻辑/本地化检查、489 项只读原生契约和 22 项 CI 工具测试通过，检查不执行 Unity 游戏代码。
