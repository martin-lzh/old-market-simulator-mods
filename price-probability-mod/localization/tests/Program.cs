...
internal static class Program
{
    ...
    private static void PendingFallbackIsNotCached()
    {
        ...
        Equal("Recommended", GameText.Native("recommended"), "fallback while loading");
        True(GameText.Stamp.EndsWith(":pending", StringComparison.Ordinal), "pending stamp");
        table.Succeed(new StringTable(new Dictionary<string, string> { ["recommended"] = "  Recommandé {natif}  " }));
        Equal("Recommandé {natif}", GameText.Native("recommended"), "loaded native value");
    ...
    private static void FailedTableUsesFallback()
    {
        ...
        Equal("Recommended", GameText.Native("recommended"), "known-key fallback after failure");
    ...
    private static void LocaleReselectionRecovers()
    {
        ...
        LocalizationSettings.StringDatabase.OnGetTable = (_, locale) =>
        {
            if (locale.Identifier.Code == "fr")
            {
                ...
                return Completed(new StringTable(new Dictionary<string, string> { ["recommended"] = "Prix conseillé rétabli" }));
            }
            return Completed(new StringTable(new Dictionary<string, string> { ["recommended"] = "Empfohlen" }));
        };

        LocalizationSettings.SelectedLocaleAsync = Locale("fr");
        Equal("fr:failed", GameText.Stamp, "first French table request");
        Equal("Recommended", GameText.Native("recommended"), "fallback after first French failure");
        LocalizationSettings.SelectedLocaleAsync = Locale("de");
        Equal("Empfohlen", GameText.Native("recommended"), "intermediate locale native value");
        LocalizationSettings.SelectedLocaleAsync = Locale("fr");
        Equal("fr:ready", GameText.Stamp, "reselected French table stamp");
        Equal("Prix conseillé rétabli", GameText.Native("recommended"), "reselected French native value");
    ...
    private static void LocaleSwitchInvalidatesCache()
    {
        LocalizationSettings.StringDatabase.OnGetTable = (_, locale) => Completed(new StringTable(
            new Dictionary<string, string> { ["recommended"] = locale.Identifier.Code == "de" ? "Empfohlen" : "Recommandé" }));
        LocalizationSettings.SelectedLocaleAsync = Locale("de");
        Equal("Empfohlen", GameText.Native("recommended"), "German native value");
        LocalizationSettings.SelectedLocaleAsync = Locale("fr");
        Equal("Recommandé", GameText.Native("recommended"), "French value after locale switch");
