...
更新前退出游戏、备份并替换旧 DLL。卸载时只删除上表位置的 `OldMarket.MaterialCost.dll`，存档不用转换。其他修改同一界面的 Mod 可能造成文字重叠；全部语言、屏幕尺寸和联机组合尚未逐一检查。
