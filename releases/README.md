...
3. Verify the ZIP's explicit file list, version, README, license, and embedded messages. Public plugin ZIPs contain only the original DLL, README, and LICENSE. Create `SHA256SUMS.txt` for the exact bytes to upload.
...
3. 检查 ZIP 文件清单、版本、README、许可证和内嵌语言资源。公开插件 ZIP 仅含原创 DLL、README 和 LICENSE；为将上传的准确文件生成 `SHA256SUMS.txt`。
