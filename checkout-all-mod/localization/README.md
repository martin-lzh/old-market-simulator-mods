...
`messages.txt` 只保存 Checkout All 自有的三条提示：长按提示、切换模式提示和长按模式提示，覆盖 Old Market Simulator 2.1.6 配置的 13 种语言。游戏已有的 `put_to_bag`、`take_pouch`、`turn_on`、`turn_off` 动作词会在运行时从当前 `Translations` 表读取；语言表尚不可用时使用英文回退，切换语言后会重新加载。
